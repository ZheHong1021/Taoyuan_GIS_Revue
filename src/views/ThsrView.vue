    ...
    <section class="flex flex-col w-full items-center">
      ...
      <div class="w-1/2 my-4">
          ...
          <Dropdown 
            ...
            :options="$store.state.new_high_Speed_Rail" 
            optionLabel="name" 
            optionValue="id"
...
export default {
   components: {
    ...
    setup(){
      ...
      const info = store.state.thsr_Choose;
    ...
    onMounted(async ()=>{
      ...
      const is_empty = store.getters.get_NewThsrArr_Length
      if(is_empty){
        setTimeout(() => {
            store.commit('setNewStation', 'thsr');
        }, 1500);
      }
    })
