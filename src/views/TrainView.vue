<template>
    ...
    <section class="flex flex-col w-full items-center">
      ...
      <form class="flex flex-col w-full items-center" @submit.prevent='Search_Info'>
...
export default {
   components: {
    ...
    setup(){
      ...
      const info = store.state.train_Choose;
    ...
    onMounted(async ()=>{
      ...
      const is_Arr_empty = store.getters.get_NewTrainArr_Length
      if(is_Arr_empty){
        setTimeout(() => {
            store.commit('setNewStation', 'train');
        }, 1500);
      }
    })
